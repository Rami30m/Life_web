...
export default function BiometricVerification({ accessToken, onSuccess, onCancel }) {
  ...
  const handleAutoCapture = async (initialDetection) => {
    ...
    try {
      ...
      // Успех! Передаем обновленные данные пользователя
      setStatus({ type: "success", message: "Биометрия успешно зарегистрирована!" });
      
      setTimeout(() => {
        onSuccess(data.user); // Передаем обновленного пользователя
      }, 1500);
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-100 to-blue-300 p-4">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-8 transition-all duration-300 text-center">
        ...
        <p className="text-gray-600 text-sm mb-6">
          Посмотрите прямо в камеру для регистрации
        </p>

        {!isModelsLoaded && (
          <div className="text-blue-600 mb-4 flex items-center justify-center gap-2">
        ...
        {isModelsLoaded && !loading && (
          <div className="mt-4 text-xs text-gray-500 bg-gray-50 p-3 rounded-lg">
            ...
            <p>Смотрите прямо в камеру. Регистрация произойдет автоматически через 3 секунды после обнаружения вашего лица.</p>
          </div>
        )}
      </div>
    </div>
